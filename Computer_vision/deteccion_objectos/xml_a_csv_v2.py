"""
Usage:
# creamos la lista para train data:
python xml_to_csv.py -i [PATH_TO_IMAGES_FOLDER]/train -o [PATH_TO_SCV_FOLDER]/train_labels.csv
# creamos la lista para test data:
python xml_to_csv.py -i [PATH_TO_IMAGES_FOLDER]/test -o [PATH_TO_SCV_FOLDER]/test_labels.csv
"""
...
def xml_to_csv(path):
    """recorremos todos los archivos .xml (generados con labelImg) y con ellos generamos un Pandas datagrame (SCV).
    Parameters:
    ----------
    path : {str}
        RUTA QUE CONTIENE los archivos .xml 
    Returns
    -------
    Pandas DataFrame
        ruta donde se generara el SCV
    """
    classes_names = []
    xml_list = []
